...
def selection(df_pop, num_parents):
    ...
    :param df_pop: dataframe of the population containing rdkit mol obj, property predictions, fitnes vals
...
def get_fitness_vals(df_pop):
    """
    performance evaluation: here, the fitness score equals the molar lignin solubility

    :param df_pop: pandas dataframe of population
    :return: df_pop: pandas dataframe of population, updated values
    """
    for iloc_ind in range(df_pop.shape[0]):
        loc_ind = df_pop.index[iloc_ind]
        fitness_value = 10**(df_pop.loc[loc_ind, 'GNN_prediction_lignin'])

        df_pop.loc[loc_ind, 'fitness value'] = fitness_value
...
def get_random_frag_ins(group_constraints):
    """
...
def get_random_frag_from_class(group_constraints):
    """
    randomly draws a class, and then, randomly draws a fragment from the class
    slight differences for AAF: here no functional that are reactive with the reaction liquor are allowed!
    ...
    """
    if group_constraints == 'AAF':
        frag_path = './fragments_group_wise_AAF.csv'
        # for windows
        df_frag = pd.read_csv(frag_path, encoding='cp1252', sep=',')
        # for linux
        # df_frag = pd.read_csv(frag_path, encoding='utf-8', sep=',')
...
def substitution(mol, group_constraints):
    """
    ...
    if group_constraints == 'AAF':
        symbol_subs = ['C', 'O', 'S', 'P']
        subs_valences = [4, 2, 4, 5]
        atomic_nums = [6, 8, 16, 15]
    else:
        symbol_subs = ['C', 'O', 'N', 'S', 'P']
        subs_valences = [4, 2, 3, 4, 5]
...
def split_ring(mol):
    ...
    if len(eligable_bonds) > 0:
        ...
        # deleting neighbor bonds does not cut the ring into frags
...
def mutation_mol_constrainer(group_constraints, max_mol_weight, flag_successful, mol, mut_mol, SA_max):
    if flag_successful == 1:
        ...
        if group_constraints == 'AAF':
            stability_bool = stability_checker(mol)
            if not stability_bool:
                flag_successful = 0
                mol = mut_mol
...
def mutation(df_pop, mutation_rate, max_mol_weight, group_constraints, SA_max):
    ...
    num_operations = 8  #addition, insertion, relocation, deletion, bond mutation, bond_addition, bond_deletion, atom subsitution
    ...
    for ilocInd in range(df_pop.shape[0]):
        ...
        if rand_mut <= mutation_rate:
            ...
            # pick random numbers to determine which operation on molecule should be performed
            rand_ints = np.array([0, 1, 2, 3, 4, 5, 6, 7])
            ...
            for rand_int in rand_ints:
                ## group addtion
                if rand_int == 0:
                    frag = get_random_frag_from_class(group_constraints)
                ...
                ## atom addition
                if rand_int == 2:
                    frag = get_random_frag_ins(group_constraints)
                ...
                ## atom substition
                if rand_int == 7:
                    mol, flag_successful = substitution(mut_mol_copy, group_constraints)
                    flag_successful, mol = mutation_mol_constrainer(group_constraints, max_mol_weight, flag_successful,
                                                                    mol, mut_mol, SA_max)
...
def stability_checker(mol):
    """
    checks for acid stability and aldehyde-reactivity conform with aldehyde-assisted fractionation (see Shuai et al.)
    ...
    num_amide = Chem.Fragments.fr_amide(mol)
    ...
    O_pattern = Chem.MolFromSmiles('O-O')
    num_O = len(list(mol.GetSubstructMatch(O_pattern)))
    ...
    # acid + aldehyde stability:
    group_nums = np.array(
        (num_ester, num_amide, num_isocyan, num_nitrile, num_hydrazide, num_hydrazide_2, num_anhydride,
         num_prim_amines, num_sec_amines, num_aldehydes, num_Ar_N, num_O, num_sulf_acid, num_OH
         ))
...
def get_all_time_fittest(df_fittest, df_pop):
    ...
    df_fittest = df_fittest.drop(df_fittest.loc[df_fittest['GNN_prediction_lignin'] <= -2.5].index)
    return df_fittest

def ini_statistics_frame():
    other_params = ['mean fitness', 'mean solubility']
...
def get_stats(df_stats, df_pop):
    ...
    stat_dict = dict.fromkeys(['mean fitness', 'mean solubility'])

    mean_fit = df_pop['fitness value'].mean()
    mean_sol = df_pop['GNN_prediction_lignin'].mean()

    stat_dict['mean fitness'] = [mean_fit]
    stat_dict['mean solubility'] = [mean_sol]
...
def post_process(df_pop, df_fittest, df_stats, df_groups, save_all_pops_flag, gen):
    ...
    fig_path = './results/figures/fittest.png'
    mol_list = df_fittest['mol'].to_list()
    sols = df_fittest['GNN_prediction_lignin'].to_list()

    try:
        img = Draw.MolsToGridImage(mol_list, legends=[str(round(sol,2)) for sol in sols], molsPerRow=5, subImgSize=(200, 200),returnPNG=False, maxMols=500)
        img.save(fig_path)
    except:
        print('Drawing of Mol Grid not possible')

    ###### print mean fitness and mean solubility #####
    gen_num = range(1,df_stats.shape[0]+1)
    mean_fitness = df_stats['mean fitness'].to_list()
    mean_sol = df_stats['mean solubility'].to_list()

    fig_path = './results/figures/mean_fit.png'
    ...
    fig_path = './results/figures/mean_sol.png'
    plt.scatter(gen_num, mean_sol, marker='o')
    plt.xlabel('number of generations')
    plt.ylabel('mean log solubility lignin')
    plt.savefig(fig_path)
    plt.close()
    ...
    for name in col_names:
        fig_path = './results/figures/%s.png' %(name)
